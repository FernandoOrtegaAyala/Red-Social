...
export default function ContenidoFuncional({ tema }: { tema: string }) {
  return (
    <>
      <div className="w-full h-auto mt-12 md:mt-0 px-6 lg:px-10 bg-background flex flex-col gap-2 items-start justify-start text-muted-foreground">
        <p className="text-sm mt-5 mb-2">Cuenta</p>
        <Link
          ...
          <p className="ml-2 py-1">Editar perfil</p>
        </Link>
        <Link
          href="/feed/settings/change-password"
          className="w-full h-10 flex flex-row items-center hover:bg-ring hover:text-white hover:rounded-md">
          <LockClosedIcon className="w-6 h-full" />
          <p className="ml-2 py-1">Cambiar contraseña</p>
        </Link>
        <ThemeButton
          tema={tema}
          texto=""
        ...
        <Link
          ...
          <p className="ml-2 py-1">Idioma</p>
        </Link>
